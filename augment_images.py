...
while i <= images_to_generate:
    ...
    image_name = images_path + "/" + temp_image_name
    ...
    # random.seed(42)
    filename = labels_path + "/" + temp_image_name.split('.')[-2] + ".txt"
    bboxes = []
    category_ids = []
    ...
    cv2.imwrite(images_path + '/' + Path(image_name).stem + 'augm.jpg',cv2.cvtColor(augmented_image, cv2.COLOR_RGB2BGR))
    # save the changed ground truth file
    augm_file = open(labels_path + '/' + Path(image_name).stem + 'augm.txt', 'w')
